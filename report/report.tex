...
The reaction stimulus test starts with three LCD dots that turn off sequentially, with a random delay between each dot turning off. Once the last dot turns off, the user must press a button, and the delay is displayed in milliseconds on eight 7-segment displays. The FPGA stores the reaction delays from the last three trials, and can display the fastest, slowest, and average reaction times. These values can be reset by the user.
...
The FPGA reaction stimulus prompt and response recording depends on four major components. These are the eight-digit counter, the ALU, the reaction countdown, and the output select and override. These components are shown in Figure \ref{project_structure}. Each of these components are enabled and disabled via a finite state machine.
...
An eight-digit five-bit-BCD counter is the primary mechanism for keeping track of the reaction time for each stimulus test to the nearest millisecond. Each BCD digit is passed into an 8x5-to-5 multiplexer. The purpose of this multiplexer is to synchronise the selected output digit with the currently active seven segment display anode, outputting each digit onto its associated display segment.
...
Clear communication of the expected inputs and outputs from each module between group members played a significant role in keeping the final assembly simple. \textit{Examples include the consistent transfer of characters to be displayed by using 8x5-to-5 multiplexers to transmit one character at a time unless otherwise required, such as for the ALU.} THIS MAKES NO SENSE
...
Since the project was significantly modularised, it was important that individual modules could be developed and tested without hindering the development of other modules. The method used to achieve this was to use git branching and merging. Individual modules were developed on separate branches, tested, and then merged into the main branch. This ensured that the main branch was always stable, and that broken changes could be easily rolled back.
...
A second benefit of each module being a ``black box'' was that since most modules interacted through standardised interfaces, how the modules operated internally was irrelevant. This allowed for significant changes within a module to how a module completed a task, without disturbing inter-module communication or the operation of other modules.
